<script lang="ts">
	...
	import { Link01Icon, Location01Icon } from "@hugeicons/core-free-icons";
...
<Container twStyles="w-[400px] md:w-[650px] px-6 flex flex-col">
  ...
  <div class="inline-flex flex-col items-start md:items-center md:flex-row-reverse justify-between">
    <span class="inline-flex items-center gap-1 text-textDim">
      <HugeiconsIcon icon={Location01Icon} size={20} color="rgba(var(--accent))"/>
    ...
    <ul class="inline-flex items-center gap-2 text-accent/60 text-sm">
      {#if experience.links}
        {#each experience.links as link}
          <a class="inline-flex items-center hover:text-accent/90 transition-all" href={link.url} target="_blank">{link.title} <HugeiconsIcon icon={Link01Icon} color="rgba(var(--accent))" size={16} strokeWidth={2}/></a>
        {/each}
      {/if}
    </ul>
